...
/**
 * The live scoring screen. Gets all data through useScorecardApi(): the real
 * database normally, or the in-memory practice stand-in on /practice.
 */
export default function ScorecardScreen({ roundId }: { roundId: string }) {
  const router = useRouter();
  const { appUser, isActive } = useAuth();
  const api = useScorecardApi();
  ...
  useEffect(() => {
    ...
    const loadRound = async () => {
      ...
      try {
        ...
        if (!r) {
          ...
          const live = await api.getLiveRound(appUser.groupId).catch(() => null);
    ...
    loadRound();
  }, [api, roundId, appUser, isActive, router]);
    ...
    const loadScorecardData = async () => {
      ...
      try {
        const [existing, activeMembers, roundRsvps] = await Promise.all([
          api.getScorecardForMarker(roundId, appUser.uid, appUser.groupId),
          api.getActiveMembers(appUser.groupId),
          api.getRoundRsvps(roundId),
        ]);
        ...
        const [playerMember, existingHoles] = await Promise.all([
          api.getMember(existing.playerId),
          api.getHoleScores(existing.id),
        ]);
  ...
  useEffect(() => {
    ...
    return api.subscribeScorecardForMarker(
      roundId,
      appUser.uid,
      (nextScorecard) => {
  ...
  useEffect(() => {
    ...
    return api.subscribeRoundRsvps(
      roundId,
      setRsvps,
  ...
  useEffect(() => {
    ...
    return api.subscribeSideClaimsForRound(
      roundId,
      setSideClaims,
      (err) => console.warn("Unable to subscribe to side claims", err)
    );
  }, [api, roundId, appUser, isActive]);

  useEffect(() => {
    if (!scorecard?.id) return;

    return api.subscribeHoleScores(
      scorecard.id,
      (nextHoles) => {
  ...
  const handleStartCard = async () => {
    ...
    setError("");
    setStarting(true);
    // The player being marked decides their tee (ladies' tee for women).
    const playerUser = eligiblePlayers.find((member) => member.uid === playerToMarkId) ?? null;
    try {
      const existingPlayerCard = await api.getScorecardForPlayer(
        round.id,
        playerToMarkId,
        appUser.groupId
      );
      if (existingPlayerCard) {
        if (existingPlayerCard.markerId === appUser.uid) {
          ...
          const existingHoles = await api.getHoleScores(existingPlayerCard.id);
          setHoles(
            existingHoles.length > 0
              ? existingHoles
              : buildInitialHoles(
                  ...
                  existingPlayerCard.courseHoles
                )
          );
          setStarting(false);
          return;
      ...
      const [playerMember, group] = await Promise.all([
        api.getMember(playerToMarkId),
        api.getGroup(appUser.groupId),
      ]);
      const groupSettings = normaliseGroupSettings(group?.settings);
      const baseHandicap = playerMember?.currentHandicap ?? 0;
      const playerTeeSet = getPlayerTeeSet(round, playerToMarkId, playerUser);
      const playerCourseHoles = getEffectiveCourseHoles(round, playerToMarkId, playerUser);
      ...
      const playingHandicap = calculatePlayingHandicap({
        ...
        gender: playerUser?.gender ?? null,
      });

      const id = await api.createScorecard({
        roundId: round.id,
        groupId: appUser.groupId,
      ...
      setScorecard(card);
      setHoles(buildInitialHoles(round, playingHandicap, playerToMarkId, playerCourseHoles));
      markSyncPending();
    } catch {
  ...
  const handleHoleChange = (holeNumber: number, gross: string) => {
    ...
    if (Number.isNaN(grossScore) || grossScore <= 0) {
      ...
      api.setHoleScore(scorecard.id, holeNumber, {
        ...cleared,
        grossScore: null,
        netScore: null,
  ...
  const handleStablefordOverride = async (
    ...
    beginHoleWrite(holeNumber);
    await api.setHoleScore(scorecard.id, holeNumber, {
      par: hole.par,
  ...
  const syncTotals = async (
    ...
    await api.updateScorecard(scorecardId, { totalGross, totalStableford });
    setScorecard((prev) =>
  ...
  const handleSignOff = async () => {
    ...
    setError("");
    try {
      await api.updateScorecard(scorecard.id, {
        status: "submitted",
        signedOff: true,
                  ...
                  {markerName && round && (
                    <>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          // No real card exists in practice mode.
                          if (!api.practice) router.push(`/rounds/${round.id}/my-card`);
                        }}
...
function buildInitialHoles(
  ...
  handicap: number,
  playerId?: string,
  // The card's own frozen holes win — they're the tee it was created on.
  cardHoles?: CourseHoleLite[] | null
): HoleScore[] {
  const layout = mapCourseHoles(
    cardHoles && cardHoles.length === 18
      ? cardHoles
      : getEffectiveCourseHoles(round, playerId)
  );
  const specialHoles = getEffectiveSpecialHoles(round);
  return layout.map((h) => ({
    holeNumber: h.number,
